function varargout = Level_1(varargin)
% LEVEL_1 MATLAB code for Level_1.fig
%      LEVEL_1, by itself, creates a new LEVEL_1 or raises the existing
%      singleton*.
%
%      H = LEVEL_1 returns the handle to a new LEVEL_1 or the handle to
%      the existing singleton*.
%
%      LEVEL_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LEVEL_1.M with the given input arguments.
%
%      LEVEL_1('Property','Value',...) creates a new LEVEL_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Level_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Level_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Level_1

% Last Modified by GUIDE v2.5 01-Dec-2023 19:09:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Level_1_OpeningFcn, ...
                   'gui_OutputFcn',  @Level_1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Level_1 is made visible.
function Level_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Level_1 (see VARARGIN)
...
% Choose default command line output for Level_1
handles.output = hObject;

% Update the text field with the initial value
handles.KpSlider = 0.0050; % Initial value
handles.TiSlider = 1/100;  % Initial value for the integral gain
handles.TdSlider = 0.05; % Initial value for the derivative gain
...
handles.Kpmax = handles.KpSlider + 1.5; handles.Kpmin =  handles.KpSlider - 1.5;
...
if handles.Kpmin <= 0
    handles.Kpmin = 0.00000;
end

if handles.Timin <= 0
    handles.Timin = 0.000001;
end

if handles.Tdmin <= 0
    handles.Tdmin = 0.000001;
end
...
% UIWAIT makes Level_1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Level_1_OutputFcn(hObject, eventdata, handles) 
...
set(handles.TdValue_edit, 'String', num2str(round(TdVal,2)));
...
handles.cost = '-'; % Initial value for the current cost
...
function closeEndTimer(~, ~, hObject)
    ...
    if handles.flag == 1
        ...
        try
        close_system('Level_1_sys',0)
        catch
        end
        %bdclose
        close(hObject)
        uiwait(Level_1_end(bestCost))
    end
